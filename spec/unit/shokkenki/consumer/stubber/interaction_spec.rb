...
describe Shokkenki::Consumer::Stubber::Interaction do
  context 'created from json' do
    ...
    before do
      allow(Shokkenki::Consumer::Stubber::Term).to receive(:from_json).with({ 'request' => 'json' }).and_return request_term
      allow(Shokkenki::Consumer::Stubber::Term).to receive(:from_json).with({ 'response' => 'json' }).and_return response_term
    end
